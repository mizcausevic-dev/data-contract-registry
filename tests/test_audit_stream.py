    ...
    async def test_emit_posts_to_events_endpoint_when_enabled(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        async with httpx.AsyncClient(transport=transport) as client:
            await audit_stream.emit(
                ...
                payload={"dataset_id": "users.daily_active", "version": "1.0.0"},
            )
        assert len(captured) == 1
        body = captured[0]
        assert body["kind"] == "contract_promoted"
        assert body["source"] == "data-contract-registry"
        assert body["payload"]["dataset_id"] == "users.daily_active"
        assert body["payload"]["version"] == "1.0.0"
        ...
        async with httpx.AsyncClient(transport=transport) as client:
            await audit_stream.emit(
                ...
                payload={
                    ...
                    "version": "1.0.0",
                    "deprecation_uri": "https://wiki/migrate",
                },
            )
        assert captured[0]["kind"] == "contract_deprecated"
